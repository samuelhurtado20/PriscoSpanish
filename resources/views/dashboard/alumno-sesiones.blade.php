                ...
                <thead>
                    ...
                    @foreach ($sesiones as $s)
                        <tr class="session-row" data-toggle="modal" data-target="#ps_session_modal" data-session="{{$s->id}}" style="cursor:hand;">
                            <td> {{$s->status}}</td>
                            <td> {{$s->offer->find($s->teacher_id)->first()->nombre}}</td>
                            <td> {{$s->language}} </td>
                            <td> {{$s->start}}</td>
                            <td> {{$s->end}}</td>
                            <td> {{ $s->teachers->find($s->teacher_id)->first()->name }}</td>
                            <td> {{$s->price}}</td>
                        </tr>
                    @endforeach
                    </tbody>
		...
		<form id="accion_sesion" name="accion_sesion" method="post">
		        ...
		        <input type="hidden" name="sesion_n" value="el_Id">
                        ...
                        <div style="min-height:40px;" class="panel panel-default">
                                ...
                                <div class="checkbox">
                                    <label>
                                            <input type="checkbox" id="completada_alumno" name="completada_alumno">Completed - Student
                                    </label>
                                </div>
